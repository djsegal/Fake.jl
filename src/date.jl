module Date
  ...
  function between_except(from, to, excepted)
    begin
  ...
  function birthday(min_age = 18, max_age = 65)
    t = ::Date.today
  ...
  function prepare_bounds(t, min_age, max_age)
    [t.year - min_age, t.year - max_age]
  end

  function handled_leap_years(top_bound, bottom_bound)
  ...
  function customized_bound(bound, increase = false)
    if (bound % 4) != 0
      bound += 1 if increase
  ...
  function get_date_object(date)
    date = ::Date.parse_yml(date) if date.is_a?(String)
